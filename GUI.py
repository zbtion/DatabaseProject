    ...
    for index, row in stock_data.iterrows():
        ...
        if (row['Close'] <= latest_buy_price * (1-threshold) and holding_share != 0) or (holding_share == 0 and row.name in golden_cross):
            ...
            if use_trailing_stop and max_profit_level > 0:
                ...
                if current_profit_percent < next_level and row.name in death_cross:
                    sell(row)
                    investment_amount = initial_investment
                    max_profit_level = 0
                    continue
...
def main():
    def on_submit():
        ...
        for code in stock_codes:
            ...
            print(code)
            record, buy_dates, sell_dates, cash, profit, rate_of_return, golden_cross, death_cross = simulate_martingale_strategy(stock_data, threshold, initial_investment, initial_cash, max_buy_times, use_trailing_stop, trailing_stop_percent, stop_profit_percent, buy_multiplier)
            print_result(stock_data, buy_dates, sell_dates, 'Martinggale', code,cash, profit, rate_of_return, golden_cross, death_cross)
